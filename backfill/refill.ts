...
function standardise(rec: DayRec, pt: string) {
  ...
  for (const ch of chains) {
    ...
    for (const [src, amt] of Object.entries(mirror[ch])) n[src] = { [pt]: amt };
  ...
  for (const ch of chains) {
    ...
    for (const [prov, dests] of Object.entries(n.bridgedTo.bridges ?? {})) {
      ...
      for (const [k, v] of Object.entries(dests as Record<string, any>)) {
        ...
        const dst =
          k === "amount" ? can(String(o.source ?? "ethereum")) : can(k);
        pMap[dst] = { amount: amt };
      }
...
(async () => {
  ...
  for (const ts of stamps) {
    ...
    /*──────── Garde-fou totalCirculating (après diff) ───────*/
    if (prev) {
      ...
      } else if (newTot > prevTot && diffPct > TOLERANCE) {
        console.log(chalk.bold("\nΔ totalCirculating"));
        console.log(
